  ...
  <p class="text-xs uppercase tracking-[0.25em] text-sky-700 font-medium">
    ...
    <section
      ...
      <div class="max-w-7xl mx-auto px-4 relative">
        <!-- Filters -->
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12" data-aos="fade-up"
          data-aos-delay="100">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="(project, index) in filtered" :key="project.id"
            ...
            @click="open(project)" data-aos="zoom-in" :data-aos-delay="index * 50">
            <div class="h-64 w-full overflow-hidden relative">
              <div
                class="absolute inset-0 bg-gradient-to-t from-black/60 via-black/0 to-black/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10">
              </div>
            ...
            <div class="p-6">
              ...
              <div class="flex items-center justify-between gap-3">
                <span
                  class="inline-flex items-center px-3 py-1.5 rounded-lg bg-sky-50 text-sky-700 text-xs font-semibold uppercase tracking-wide">
                  {{ project.eventType }}
                </span>
                <span
                  class="inline-flex items-center px-3 py-1.5 rounded-lg bg-slate-100 text-slate-700 text-xs font-medium">
                  {{ project.division }}
                </span>
              </div>
            </div>
        ...
        <!-- Empty state -->
        <div v-if="filtered.length === 0" class="text-center py-20" data-aos="fade-up">
    ...
    <transition enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="show" class="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4"
        @click.self="close">
        <transition enter-active-class="transition-all duration-300 ease-out"
          leave-active-class="transition-all duration-200 ease-in" enter-from-class="opacity-0 scale-95 translate-y-4"
          leave-to-class="opacity-0 scale-95 translate-y-4">
          <div v-if="show" class="max-w-5xl w-full bg-white rounded-3xl overflow-hidden shadow-2xl transform">
            <div class="relative">
              ...
              <div class="absolute bottom-0 left-0 right-0 p-8 z-20">
                <div class="flex items-center gap-3 mb-3">
                  <span
                    class="inline-flex items-center px-4 py-2 rounded-full bg-white/20 backdrop-blur-md text-white text-sm font-semibold border border-white/30">
                    {{ selected.eventType }}
                  </span>
                  <span
                    class="inline-flex items-center px-4 py-2 rounded-full bg-white/20 backdrop-blur-md text-white text-sm font-medium border border-white/30">
                    {{ selected.division }}
                  </span>
                </div>
              </div>
