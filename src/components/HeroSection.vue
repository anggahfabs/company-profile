<template>
  <section
    ...
    <div v-for="(slide, index) in slides" :key="index"
      class="absolute inset-0 bg-cover bg-center transition-all duration-1000 ease-out" :style="{
      ...
      <div class="relative w-full h-full flex flex-col items-center justify-center text-white px-6 text-center z-10">
        <!-- Animated badge -->
        <div class="transition-all duration-700 delay-100 mb-6" :class="{
        ...
        <h1
          ...
          <span
            class="inline-block bg-gradient-to-r from-white via-sky-100 to-white bg-clip-text text-transparent animate-shimmer">
            {{ slide.title }}
          </span>
        </h1>

        <!-- Subtitle with staggered animation -->
        <p class="max-w-3xl text-xl md:text-2xl transition-all duration-700 delay-300 text-white/90 leading-relaxed"
          :class="{
            'opacity-100 translate-y-0': currentIndex === index && showContent,
        ...
        <!-- CTA Buttons -->
        <div class="flex flex-col sm:flex-row gap-4 mt-10 transition-all duration-700 delay-400" :class="{
          'opacity-100 translate-y-0': currentIndex === index && showContent,
          'opacity-0 translate-y-8': currentIndex !== index || !showContent,
        }">
        ...
        <!-- Scroll indicator -->
        <div class="absolute bottom-12 left-1/2 -translate-x-1/2 transition-all duration-700 delay-500" :class="{
          'opacity-100 translate-y-0': currentIndex === index && showContent,
          'opacity-0 translate-y-4': currentIndex !== index || !showContent,
        }">
    ...
    <div class="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-3 z-20">
      <button v-for="(slide, index) in slides" :key="index" class="group relative transition-all duration-300"
        :class="currentIndex === index ? 'w-12' : 'w-3'" @click="goTo(index)" aria-label="`Go to slide ${index + 1}`">
        <!-- Background dot -->
        <div class="h-3 rounded-full transition-all duration-300" :class="currentIndex === index
          ? 'bg-gradient-to-r from-sky-400 to-sky-600 shadow-lg shadow-sky-500/50'
          : 'bg-white/30 hover:bg-white/50 group-hover:scale-110'"></div>
    ...
    <div class="absolute top-8 right-8 z-20">
      <div
        class="px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-white font-semibold text-sm">
...
const fetchSlides = async () => {
  try {
    ...
    slides.value = [
      {
        id: 1,
        title: "Memorable Events Creation",
        subtitle: "Event planning & production for concerts, corporate, and private events.",
