<template>
  <section class="py-24 left-1/2 right-1/2 w-screen -translate-x-1/2 relative overflow-hidden bg-gradient-to-b from-white via-slate-50 to-white">
    ...
    <div class="absolute inset-0 opacity-30 pointer-events-none" aria-hidden="true">
      ...
      <div class="absolute top-1/2 left-1/2 w-96 h-96 bg-purple-200 rounded-full mix-blend-multiply filter blur-3xl animate-blob animation-delay-4000"></div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 relative">
      ...
      <div class="text-center mb-16" data-aos="fade-up">
        ...
        <h2 class="text-4xl md:text-5xl font-bold mb-4 tracking-tight bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 bg-clip-text text-transparent">
          Layanan Kami
        </h2>
        ...
        >
          <template #default>
            <div class="mt-4 flex items-center justify-between">
              <div class="relative">
                <div class="absolute inset-0 bg-gradient-to-br from-sky-500/20 to-amber-500/20 rounded-2xl blur-xl group-hover:blur-2xl transition-all duration-500 opacity-0 group-hover:opacity-100"></div>
                <span class="relative text-5xl md:text-6xl transform group-hover:scale-110 transition-transform duration-500 inline-block group-hover:rotate-12">
                  {{ service.icon }}
                </span>
              </div>
            ...
            <div class="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-sky-500 via-amber-500 to-purple-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left rounded-b-xl"></div>
          </template>
        </BaseCard>
      </div>
      ...
      <div class="mt-20 text-center" data-aos="fade-up" data-aos-delay="400">
        <div class="inline-flex flex-col sm:flex-row items-center gap-4 p-8 bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl shadow-2xl border border-slate-700">
    ...
    >
      <div class="space-y-4">
        <div class="flex items-start gap-4 p-4 bg-sky-50 rounded-xl border border-sky-100">
          ...
          <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-sky-100 text-sky-700 text-xs font-semibold mb-2">
              {{ activeService.badge }}
            </div>
        ...
        <div v-if="activeService.points?.length" class="bg-white rounded-xl p-5 border border-slate-200">
          ...
          <ul class="space-y-3">
            <li 
              v-for="(point, idx) in activeService.points" 
              :key="point"
              class="flex items-start gap-3 text-sm text-slate-700"
            >
              <span class="flex-shrink-0 w-6 h-6 rounded-full bg-gradient-to-br from-sky-500 to-sky-600 text-white text-xs font-bold flex items-center justify-center mt-0.5">
                {{ idx + 1 }}
              </span>
