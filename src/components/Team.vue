<template>
  <section class="py-24 left-1/2 right-1/2 w-screen -translate-x-1/2 relative bg-white overflow-hidden">
    <div class="max-w-7xl mx-auto px-4 text-center mb-16 relative">
      <div data-aos="fade-up"
        ...
        <span class="text-xs uppercase tracking-wider text-slate-700 font-semibold">Our Team</span>
      </div>
      ...
      <p data-aos="fade-up" data-aos-delay="200"
        ...
        Our professional team is ready to bring your event to life — from planning
        to field execution.
      </p>
    </div>
    ...
    <!-- Full-bleed carousel -->
    <div class="relative">
      <div ref="viewport" class="overflow-hidden" @mouseenter="pause" @mouseleave="play">
        <div ref="track" class="flex transition-transform duration-700 ease-out"
          :style="{ transform: `translateX(-${translateX}px)` }">
          <div v-for="(member, idx) in team" :key="member.id" :data-aos-delay="idx * 60" class="flex-shrink-0 px-4"
            ...
            <div
              ...
              :style="{ maxWidth: itemWidth + 'px' }" @click="onMemberClick(member)">
              <!-- Image container with overlay -->
              <div class="relative w-full overflow-hidden" :style="{ height: imageHeight + 'px' }">
                <!-- Gradient overlay on hover -->
                <div
                  class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10">
                </div>
                ...
                <div
                  class="absolute top-4 right-4 z-20 flex flex-col gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transform translate-x-0 md:translate-x-4 md:group-hover:translate-x-0 transition-all duration-500">
                  <!-- Instagram -->
                  <a v-if="member.instagram && member.instagram !== '#'" :href="member.instagram" target="_blank"
                    ...
                    @click.stop>
                    <svg class="w-5 h-5 text-slate-700 group-hover/ig:text-white transition-colors" fill="currentColor"
                      viewBox="0 0 24 24">
                      <path
                  ...
                  <!-- TikTok -->
                  <a v-if="member.tiktok && member.tiktok !== '#'" :href="member.tiktok" target="_blank"
                    rel="noopener noreferrer"
                    ...
                    @click.stop>
                    <svg class="w-5 h-5 text-slate-700 group-hover/tt:text-white transition-colors" fill="currentColor"
                      viewBox="0 0 24 24">
                      <path
                ...
                <div
                  class="absolute bottom-4 left-4 right-4 z-20 opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-500">
                  <div
                    class="inline-flex items-center px-3 py-1.5 rounded-full bg-white/20 backdrop-blur-md border border-white/30">
              ...
              <div class="p-5 text-center relative">
                <h3
                  class="font-bold text-xl text-slate-900 mb-1 group-hover:text-sky-600 transition-colors duration-300">
                  {{ member.name }}
                </h3>
                ...
                <p v-if="member.bio"
                  class="text-xs text-slate-600 leading-relaxed opacity-0 group-hover:opacity-100 max-h-0 group-hover:max-h-20 transition-all duration-500 overflow-hidden">
                  {{ member.bio }}
                </p>

                <!-- View profile indicator -->
                <div
                  class="flex items-center justify-center gap-2 mt-3 text-sky-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              ...
              <!-- Bottom accent line -->
              <div
                class="h-1 bg-gradient-to-r from-sky-500 via-amber-500 to-purple-500 transform scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left">
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="absolute left-1/2 transform -translate-x-1/2 bottom-6 flex gap-2 z-30">
        <button v-for="i in pages" :key="'page-' + i" @click="goToPage(i - 1)" :class="[
  ...
  <!-- Team Detail Modal -->
  <BaseModal v-model="showModal" :title="selectedMember?.name" :subtitle="selectedMember?.role">
    <div v-if="selectedMember" class="flex flex-col md:flex-row gap-6">
      <div class="w-full md:w-1/2 lg:w-2/5 flex-shrink-0">
        <div class="relative rounded-xl overflow-hidden shadow-md aspect-[3/4]">
      ...
      <div class="flex-1">
        <h4 class="text-lg font-bold text-slate-900 mb-3">About</h4>
        <p class="text-slate-600 leading-relaxed mb-6 whitespace-pre-line">
          {{ selectedMember.bio || 'No description available for this team member.' }}
        </p>

        <div
          v-if="(selectedMember.instagram && selectedMember.instagram !== '#') || (selectedMember.tiktok && selectedMember.tiktok !== '#')">
          <h4 class="text-sm font-semibold text-slate-900 mb-3 uppercase tracking-wider">Social Media</h4>
          <div class="flex gap-3">
            <!-- Instagram -->
            <a v-if="selectedMember.instagram && selectedMember.instagram !== '#'" :href="selectedMember.instagram"
              target="_blank" rel="noopener noreferrer"
              class="w-10 h-10 rounded-full bg-slate-100 hover:bg-gradient-to-br hover:from-purple-500 hover:via-pink-500 hover:to-orange-400 flex items-center justify-center transition-all duration-300 group/ig">
              <svg class="w-5 h-5 text-slate-600 group-hover/ig:text-white transition-colors" fill="currentColor"
                viewBox="0 0 24 24">
                <path
                  d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
              </svg>
            ...
            <!-- TikTok -->
            <a v-if="selectedMember.tiktok && selectedMember.tiktok !== '#'" :href="selectedMember.tiktok"
              target="_blank" rel="noopener noreferrer"
              class="w-10 h-10 rounded-full bg-slate-100 hover:bg-black flex items-center justify-center transition-all duration-300 group/tt">
              <svg class="w-5 h-5 text-slate-600 group-hover/tt:text-white transition-colors" fill="currentColor"
                viewBox="0 0 24 24">
                <path
                  d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-5.2 1.74 2.89 2.89 0 012.31-4.64 2.93 2.93 0 01.88.13V9.4a6.84 6.84 0 00-1-.05A6.33 6.33 0 005 20.1a6.34 6.34 0 0010.86-4.43v-7a8.16 8.16 0 004.77 1.52v-3.4a4.85 4.85 0 01-1-.1z" />
              </svg>
    ...
    team.value = [
      {
        ...
        bio: "Leading creative concepts and experience design for major events.",
      ...
      },
      {
        ...
        bio: "Responsible for all aspects of technical production and AV.",
