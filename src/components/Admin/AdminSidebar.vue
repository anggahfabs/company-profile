<template>
  <aside :class="[
    ...
    <nav class="flex-1 overflow-y-auto px-2 space-y-1">
      <!-- ACCORDION: CONTENT -->
      <div>
        <button @click="toggleAccordion('content')"
          ...
          <ChevronDownIcon v-if="!collapsed" :class="[
      ...
      <!-- ACCORDION: SETTINGS -->
      <div>
        <button @click="toggleAccordion('settings')"
          class="flex items-center w-full p-3 rounded-lg hover:bg-gray-100 transition">
        ...
        <transition name="slide">
          <div v-if="accordion.settings" class="ml-8 space-y-1">
            ...
            <RouterLink to="/admin/messages"
              ...
              <!-- BADGE JUMLAH PESAN -->
              <span v-if="!collapsed && unreadCount > 0"
...
export default {
  props: {
    ...
    unreadCount: {
      type: Number,
      default: 0, // ✅ ini dari parent, jangan bikin ref lagi
    },
  },
  ...
  setup(props) {
    ...
    // fetchUnreadCount tidak perlu update unreadCount karena sekarang prop
    onMounted(() => {
    ...
    const decrementUnread = () => {
      // ❌ hapus akses unreadCount.value, parent yang handle update
    };
    ...
    return {
      ...
      toggleCollapse,
      toggleAccordion,
      // ✅ props unreadCount tetap diakses di template
    };
  },
};
</script>
